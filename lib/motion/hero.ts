...
import { canRenderHeavyScene, one, prefersReducedMotion } from "./dom";
...
/** How far past its resting size the poster starts. Small — this is a settle, not a swoop. */
const POSTER_INTRO_SCALE = 1.16;

/**
 * Eases the static hero out of a push-in as the boot panel lifts.
 *
 * Without it the poster is simply there the moment the loader clears, which reads as a
 * still where the live scene reads as an arrival. This gives the same beat the scene's
 * opening camera move gives everyone else, for one transform on a static image — no WebGL,
 * nothing decoded, and it stays on the compositor.
 *
 * clearProps hands the transform back to CSS when the tween finishes, so the mobile layout's
 * translateX and the scroll scrub that follows are not fighting a leftover inline matrix.
 */
function playPosterIntro(): void {
  const wrap = one("#splineWrap");
  if (!wrap || wrap.dataset.scene !== "off") return;

  // unlockLoad() calls releaseSpline() on the reduced-motion path too, and a gsap tween is not
  // touched by the CSS reduced-motion block — it has to be declined here.
  if (prefersReducedMotion()) return;

  gsap.fromTo(
    wrap,
    { scale: POSTER_INTRO_SCALE },
    { scale: 1, duration: 1.8, ease: "power3.out", clearProps: "scale" },
  );
}
...
export function startSpline(reduced: boolean): Promise<void> {
  ...
  // Memory-constrained devices never touch the scene: no fetch, no Application, no GL context.
  // They get a captured frame of it instead, eased out of a push-in as the boot panel lifts —
  // see playPosterIntro(). Resolving immediately is the correct signal to the loader: the boot
  // animation still plays out in full (runLoader never cuts phase 1 short), it just does not
  // wait for a scene that is not coming.
...
export function releaseSpline(): void {
  ...
  // No runtime means the hero is the captured frame — there is no scene to start, but the
  // reveal should still move, so the poster eases out of its push-in instead.
  if (!app) {
    playPosterIntro();
    return;
  }
